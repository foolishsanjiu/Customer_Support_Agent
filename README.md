...
P0、P1 和本轮 P2 核心范围均已完成。当前实现包含完整业务链路、异步执行、人工审批、
故障恢复、可观测性、安全回归、真实模型评测，以及一个用于跑通客户侧流程的小型聊天页。
P2 新增的重点是上下文组装、失败归因，以及受限的诊断、修复和重新规划闭环。
...
| 应用代码覆盖率 | 91.02% |
| P2 上下文与修复护栏 | 11/11 |
| 最终提交真实模型 smoke | 7/7，任务成功率与工具选择准确率均为 100% |
| P1 完整真实模型 benchmark | 147/150，任务成功率与工具选择准确率均为 98% |
| 确定性安全评测 | 20/20 |
| 关键安全事件 | 0 |

最终提交 `8c3598b` 已通过 CI 和受保护的真实模型 smoke；150 条用例的完整 benchmark 结果
来自提交 `95daf5a`。模型、供应商指纹、数据集和评分器都会写入报告，避免把模型版本变化
误判为代码回归。完整证据见 [P1 完成审计](docs/p1-completion-audit.md)和
[评测基线说明](docs/evaluation-baseline.md)。

...
@celery_app.task(name="resolvex.run_agent")
def run_agent(run_id: int) -> None:
    _run(_with_runtime(lambda settings, sessions, saver: _start(run_id, settings, sessions, saver)))
...
async def _start(run_id, settings, sessions, saver) -> None:
    ...
    action = await guard.acquire(run_id, RunTrigger.START, checkpoint_exists=False)
    if action is not RunAction.START:
        return
    store = DatabaseAgentStore(sessions)
    ticket_id, customer_id = await store.get_run_context(run_id)
    runner = AgentRunner(
